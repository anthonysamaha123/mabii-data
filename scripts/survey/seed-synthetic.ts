...
async function run() {
  ...
  await fs.writeFile(f, "", "utf-8"); // reset
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    for (const q of personQs) {
      ...
      // bias a few answers so the output looks realistic (e.g. mostly frozen deposits)
      let val: string;
  ...
  console.log(`[seed-synthetic] wrote ${n} synthetic person submissions to wave ${WAVE}`);
}
